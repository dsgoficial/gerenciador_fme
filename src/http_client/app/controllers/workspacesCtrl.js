(function () {
  ...
  var workspacesCtrl = function ($scope, $uibModal, dataFactory, DTOptionsBuilder) {

    $scope.tabelas = []
    $scope.versoes = []
    $scope.categorias = []
    $scope.reload = function () {
      dataFactory.getTabelasVersao()
        .then(function success(response) {
         $scope.versoes = dataFactory.tabelasVersao;
      }, function error(response) {
        //FIXME
      });
      dataFactory.getTabelas()
        .then(function success(response) {
    ...
    $scope.saveWorkspace = function(data) {
        ...
        dataFactory.updateWorkspace(data)
            .then(function() {
                $scope.reload();
            })            
            .catch(function() {
  ...
  workspacesCtrl.$inject = ['$scope', '$uibModal', 'dataFactory', 'DTOptionsBuilder'];

  angular.module('fmeApp')
  .controller('workspacesCtrl', workspacesCtrl);
