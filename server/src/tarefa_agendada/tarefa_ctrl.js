...
controller.insertCron = async (usuarioUuid, nome, rotinaId, configuracao, parametros, dataInicio, dataFim) => {
  return db.conn.tx(async t => {
    ...
    await t.none(`INSERT INTO fme.tarefa_agendada_cron(uuid, nome, rotina_id, data_agendamento, usuario_id, configuracao_cron, parametros, data_inicio, data_fim) 
...
controller.insertData = async (usuarioUuid, nome, rotinaId, configuracao, parametros) => {
  return db.conn.tx(async t => {
    const usuario = await t.oneOrNone('SELECT id FROM dgeo.usuario WHERE uuid = $<usuarioUuid>', { usuarioUuid })
    ...
    await t.none(`INSERT INTO fme.tarefa_agendada_data(uuid, nome, rotina_id, data_agendamento, usuario_id, data_execucao, parametros) 
